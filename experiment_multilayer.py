...
def ablation_steering_hook(
    ...
    Formula: act' = act - α * steering_vector  (direction=-1, ablation)
             act' = act + α * steering_vector  (direction=+1, amplification)

    With centering:
        centered = act - global_mean
        centered += direction * α * steering_vec
    ...
    Args:
        ...
        alpha: Scaling factor (1.0 = 1x mean difference)
        use_centering: Whether to center before/after intervention
        direction: -1 for ablation (subtract), +1 for amplification (add)
    """
    if use_centering:
        centered = resid_post - global_mean
        centered += direction * alpha * steering_vec
