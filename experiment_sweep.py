...
Design:
  ...
  x 23 layer configs (12 singles + 5 pairs + 3 triples + 3 multi)
  x 3 concepts (time, place, tools)
  = 1,242 ablation conditions + 54 amplification controls = 1,296 total
...
# Pairs (5)  — non-overlapping consecutive pairs (L10,L11 omitted to keep 5)
...
# Multi (3)
LAYER_CONFIGS["half_early"] = list(range(0, 6))
LAYER_CONFIGS["half_late"] = list(range(6, 12))
...
def ablation_hook(resid_post, hook, steering_vec, alpha, direction):
    ...
    """
    resid_post += direction * alpha * steering_vec
    return resid_post
...
def main():
    ...
    print(f"  Layer configs:  {len(LAYER_CONFIGS)} "
          f"(12 singles + 5 pairs + 3 triples + 3 multi)")
