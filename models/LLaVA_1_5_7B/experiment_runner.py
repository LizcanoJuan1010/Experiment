...
Runs ablation experiments across all factor combinations:
  - 11 Concepts (camel, bee, horse, penguin, violin, bird, spider, candle,
                  tabby_cat, golden_retriever, labrador_retriever)
  - 1 Technique (projection)
  - 3 Layers (16=50%, 24=75%, 27=84%)
  - 5 Intensities (3.0, 6.0, 10.0, 15.0, 20.0)
...
Supports checkpoint/resume: saves progress after each condition
so the experiment can survive crashes and OOM errors.
...
# Checkpoint files
CHECKPOINT_FILE = os.path.join(RESULTS_DIR, "factorial_checkpoint.json")
SPEC_CHECKPOINT_FILE = os.path.join(RESULTS_DIR, "specificity_checkpoint.json")
...
def condition_key(concept, cav_method, layer, technique, alpha):
    """Unique string key for a factorial condition."""
    return f"{concept}|{cav_method}|{layer}|{technique}|{alpha}"


def spec_condition_key(ablated_concept, measured_concept):
    """Unique string key for a specificity condition."""
    return f"{ablated_concept}|{measured_concept}"


def load_checkpoint(path):
    """Load checkpoint file, return (completed_keys set, results list)."""
    if os.path.exists(path):
        with open(path, "r") as f:
            ckpt = json.load(f)
        return set(ckpt.get("completed_keys", [])), ckpt.get("results", [])
    return set(), []


def save_checkpoint(path, completed_keys, results):
    """Save checkpoint atomically."""
    tmp = path + ".tmp"
    with open(tmp, "w") as f:
        json.dump({"completed_keys": list(completed_keys), "results": results}, f)
    if os.path.exists(path):
        os.replace(tmp, path)
    else:
        os.rename(tmp, path)
...
def main():
    ...
    print(f"\n{sep}")
    print(f"  Experiment Runner — LLaVA-1.5 7B Projection Ablation{pilot_str}")
    ...
    # Try to load existing baselines to avoid recomputing
    if os.path.exists(baseline_path):
        print(f"\n  Loading cached baselines from {baseline_path}...")
        with open(baseline_path, "r") as f:
            baselines = json.load(f)
        # Check if baselines cover all current concepts
        missing = [c for c in CONCEPTS if c not in baselines]
        if missing:
            print(f"  Missing baselines for: {missing} — recomputing all...")
            baselines = {}
    else:
        baselines = {}
    ...
    # Pre-compute R2b baseline embeddings for all concepts
    print(f"\n  Pre-computing R2b baseline embeddings...")
    r2b_baseline_cache = {}
    for concept in CONCEPTS:
        r2_data = data["r2_benchmark"][concept]
        r2b_baseline_cache[concept] = compute_r2b_baselines(
            model, processor, r2_data
        )
    print(f"  Done ({len(r2b_baseline_cache)} concepts cached)")

    # ===================================================================
    # PHASE 2: FACTORIAL EXPERIMENT (with checkpoint/resume)
    # ===================================================================
    print(f"\n{'='*50}")
    print("PHASE 2: FACTORIAL EXPERIMENT")
    print(f"{'='*50}")

    # Build list of all conditions (excluding alpha=0.0)
    all_conditions = []
    for concept in CONCEPTS:
                ...
                for technique in TECHNIQUES:
                    for alpha in INTENSITIES:
                        ...
                        all_conditions.append(
                            (concept, cav_method, layer, technique, alpha)
                        )
    ...
    # Load checkpoint
    completed_keys, results_rows = load_checkpoint(CHECKPOINT_FILE)
    if completed_keys:
        print(f"  Resuming from checkpoint: {len(completed_keys)}/{total_conditions} done")

    condition_num = 0
    for concept, cav_method, layer, technique, alpha in all_conditions:
        condition_num += 1
        key = condition_key(concept, cav_method, layer, technique, alpha)
        ...
        try:
            ...
            save_checkpoint(CHECKPOINT_FILE, completed_keys, results_rows)
    ...
    # ===================================================================
    # PHASE 3: SPECIFICITY TEST (with checkpoint/resume)
    # ===================================================================
    print(f"\n{'='*50}")
    ...
    print(f"  Config: layer={spec_layer}, method={spec_method}, "
          f"technique={spec_technique}, alpha={spec_alpha}")

    # Load specificity checkpoint
    spec_completed, spec_rows = load_checkpoint(SPEC_CHECKPOINT_FILE)
    if spec_completed:
        print(f"  Resuming from checkpoint: {len(spec_completed)} done")

    total_spec = len(CONCEPTS) * len(CONCEPTS)
    spec_num = 0
    ...
    for ablated_concept in CONCEPTS:
        ...
        for measured_concept in CONCEPTS:
            spec_num += 1
            key = spec_condition_key(ablated_concept, measured_concept)

            if key in spec_completed:
                continue
            ...
            try:
                ...
                spec_completed.add(key)

                save_checkpoint(SPEC_CHECKPOINT_FILE, spec_completed, spec_rows)

                print(f"  [{spec_num}/{total_spec}] "
                      f"Ablate {ablated_concept.upper()} -> "
    ...
    # Specificity ratio
    if len(df_spec) > 0:
        # Ensure on_target is boolean (checkpoint may store as string)
        df_spec["on_target"] = df_spec["on_target"].apply(
    ...
    for ckpt in [CHECKPOINT_FILE, SPEC_CHECKPOINT_FILE]:
