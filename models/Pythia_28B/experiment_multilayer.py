...
Input:  experiment_data.json, model pythia-2.8b
...
N_LAYERS = cfg.N_LAYERS_TOTAL  # Pythia 2.8B has 32 layers (0-31)
ALL_LAYERS = list(range(N_LAYERS))

# Intervention modes — proportional mapping for 32-layer model
# single_L8: ~25% depth (was single_L3 at 25% of 12)
# multi_early: ~25-40% depth (was [3,4,5] in 12-layer model)
# multi_all: all layers
MODES = {
    "single_L8": [8],
    "multi_early": [8, 10, 13],
...
# ---------------------------------------------------------------------------
# Phase 0a: Extract SVM CAVs (un-normalized) for all layers
# ---------------------------------------------------------------------------
    ...
    for concept in CONCEPTS:
        ...
        for layer in ALL_LAYERS:
            ...
            if (os.path.exists(steer_path) and
                ...
                sv_c = torch.load(centered_path, weights_only=True)
...
def measure_concept_removal_multilayer(model, concept, layers, alpha, centered,
    ...
    for layer in layers:
        ...
        sv_unit = (sv / sv.norm()).cpu().float().numpy()

        with torch.no_grad():
            _, cache = model.run_with_cache(probe_text, names_filter=[hook_name])
            if token_method == "mean":
                act = cache[hook_name][0].mean(dim=0).cpu().float().numpy()
            else:
...
def main():
    ...
    print(f"  Layers:    0-{N_LAYERS - 1} ({N_LAYERS} total)")
    ...
    for concept in CONCEPTS:
        for layer in [0, 8, 16, 24, N_LAYERS - 1]:
