...
def tcav_score_from_acts(activations, cav):
    ...
    """
    acts_np = activations.float().numpy() if isinstance(activations, torch.Tensor) else activations
    cav_np = cav.float().numpy() if isinstance(cav, torch.Tensor) else cav
...
def tcav_permutation_test(pos_acts, neg_acts, cav, method="svm",
    ...
    if method == "mean_diff":
        ...
        try:
            ...
            real_cav = cav.float().numpy() if isinstance(cav, torch.Tensor) else cav
    else:
