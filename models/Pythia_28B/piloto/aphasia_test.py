...
# Add parent directory to sys.path so we can import experiment_config
# when running from test/ subdirectory
...
# Resolve paths relative to project root (parent dir), not test/ dir
...
MULTI_LAYERS = cfg.EXTRACTION_LAYERS   # All three combined
TECHNIQUES = ["subtraction", "projection"]
ALPHAS = [1.0, 3.0, 5.0, 8.0, 10.0, 15.0, 20.0]
...
def main():
    ...
    for method in CAV_METHODS:
        ...
        for layer in SINGLE_LAYERS:
            ...
            cavs[method][layer] = torch.load(path, weights_only=True).to(device)
            print(f"  Loaded: {path}")
