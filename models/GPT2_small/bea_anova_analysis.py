...
def load_json_to_df(json_path, test_type):
    ...
    for r in results:
        ...
        # Clean layer name (e.g. from 'L6' to 6)
...
def interaction_plot(data, dv, factor1, factor2, title, save_path=None):
    ...
    LABELS_ES = {
        "cav_method": "Método CAV", "layer": "Capa", "alpha_cat": "Alfa (α)",
        "delta_accuracy": "Δ Exactitud", "delta_auc": "Δ AUC",
    }
    ax.set_xlabel(LABELS_ES.get(factor1, factor1))
    ax.set_ylabel(LABELS_ES.get(dv, dv))
    ax.set_title(title)
    ax.legend(title=LABELS_ES.get(factor2, factor2))
...
def main():
    print("=" * 60)
    print("ANOVA Analysis — BEA Test Suite")
    print("=" * 60)
    ...
    TEST_NAMES_ES = {
        "synonym":    "Sinónimos",
        "naming":     "Nombramiento",
        "oddoneout":  "Elemento Diferente",
    }
    DV_NAMES_ES = {
        "delta_accuracy": "Δ Exactitud",
        "delta_auc":      "Δ AUC",
    }
    ...
    def process_test(df, dv, test_name):
        ...
        test_es = TEST_NAMES_ES.get(test_name, test_name.capitalize())
        dv_es   = DV_NAMES_ES.get(dv, dv)

        df_target = df[df["on_target"] == True].copy()
        print(f"  Rows on-target: {len(df_target)}")

        print("  [ANOVA 1] Technique x Layer (Alpha=3.0)")
        df_a1 = df_target[df_target["alpha"] == 3.0].copy()

        if len(df_a1) > 0:
            res_a1 = run_twoway_anova(df_a1, dv, "cav_method", "layer", filter_desc=f"{test_es}: Método x Capa")
            ...
            interaction_plot(
                ...
                f"{test_es} (α=3.0): {dv_es} Media por Capa",
                save_path=os.path.join(FIGURES_DIR, f"{test_name}_interaction_method_layer.png")
            )

        print("  [ANOVA 2] Technique x Alpha (Layer=6)")
        df_a2 = df_target[df_target["layer"] == 6].copy()
        df_a2["alpha_cat"] = df_a2["alpha"].astype(str)

        if len(df_a2) > 0:
            res_a2 = run_twoway_anova(df_a2, dv, "cav_method", "alpha_cat", filter_desc=f"{test_es}: Método x Alfa")
            ...
            interaction_plot(
                ...
                f"{test_es} (Capa=6): {dv_es} Media por Alfa (α)",
                save_path=os.path.join(FIGURES_DIR, f"{test_name}_interaction_method_alpha.png")
            )

        # C. Dose-Response (Alpha scaling)
        print("  [ANOVA 3] Dose-Response Alpha scaling (SVM, Layer 6)")
        df_a3 = df_target[(df_target["layer"] == 6) & (df_target["cav_method"] == "svm")].copy()
