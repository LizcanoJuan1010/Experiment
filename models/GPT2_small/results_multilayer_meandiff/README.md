...
| Archivo | Descripcion |
|---------|-------------|
| `results_factorial.csv` | 39 filas: R1, R2, deltaR1, deltaR2 por condicion |
...
- **Mejor deltaR1**: +0.70 (TIME, multi_all/alpha=4.0) — mas fuerte que SVM (+0.33)
...
|---------|-----------|-----|
| Mejor deltaR1 | **+0.70** | +0.33 |
| Validacion CV | No disponible | 80-91% |
| Cohen's d | No disponible | 3.1-4.9 |
| Rigor estadistico | Bajo | **Alto** |

Mean-diff produce efectos mas fuertes pero sin metricas de validacion. SVM es el enfoque recomendado para la tesis, pero esta comparacion es un hallazgo en si mismo: demuestra el trade-off entre potencia bruta y rigor metodologico.

## Generado por

- [`experiment_multilayer.py`](../experiment_multilayer.py) (variante mean-diff)
- [`test/aphasia_test_steering.py`](../test/aphasia_test_steering.py) (test cualitativo)
