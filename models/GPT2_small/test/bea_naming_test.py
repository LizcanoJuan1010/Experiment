"""
CCT Test — Camel and Cactus Test (Confrontation Naming with Ablation)
=====================================================================
Simulates the BEA Confrontation Naming neuropsychological test on
GPT-2 small with concept ablation via CAV projection.
...
Design:
  ...
  Layer conditions: EXPERIMENT_LAYERS (individual) + all layers (0-11)
  For each ablation condition, measures accuracy on ALL concept benchmarks
  (on-target + off-target) to test specificity.

Input:  Benchmark_construction/output/bea_naming_benchmark.json
        cavs/*.pt (pre-trained CAVs)
Output: results/bea_naming_test_results.json

Usage (inside Docker):
    docker run --gpus all -it tesis-gpt2 python -m test.bea_naming_test

Prerequisites:
    - Run run_pipeline.sh first (generates experiment_data.json + CAVs)
    - BEA association benchmark must exist in Benchmark_construction/output/
"""
...
# Ensure parent directory is in path (needed when running as python -m test.bea_naming_test)
...
from metrics import evaluate_bea_naming
...
SINGLE_LAYERS = cfg.EXPERIMENT_LAYERS
ALL_LAYERS = cfg.EXPERIMENT_LAYERS_ALL
ALPHAS = cfg.INTENSITIES

BENCHMARK_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..",
    "Benchmark_construction", "output", "bea_naming_benchmark.json",
)
RESULTS_DIR = cfg.RESULTS_DIR
OUTPUT_FILE = os.path.join(RESULTS_DIR, "bea_naming_test_results.json")
...
def evaluate_with_hooks(model, dataset, fwd_hooks):
    """Evaluate BEA association benchmark with ablation hooks active."""
    if not fwd_hooks:
        return evaluate_bea_naming(model, dataset)
...
def main():
    ...
    print("=" * 60)
    print("CCT Test — Confrontation Naming with Ablation")
    print("=" * 60)
    ...
    # Naming and OddOneOut use 3 broad categories: "time", "place", "tools"
    # We will map our CONCEPTS to these categories based on semantic field to find relevant ones.
    category_mapping = {
        "tools": ["lock", "toothbrush", "violin", "candle", "fire", "snowman"], # Objects/tools
        "place": ["king", "knight", "soldier"], # People that exist in places/castles
        "time": ["musician", "baker", "barber"] # Generic fallback to test off-target
    }
    ...
    # We keep only concepts that fall into our mapped categories
    ...
    # Evaluate using the full category list for each concept
    items_by_concept = {}
    for c in available:
    ...
    baselines = {}
    correct_items_by_concept = {}  # Only items the model answers correctly

    for concept in available:
        acc, per_item = evaluate_bea_naming(model, items_by_concept[concept])
        baselines[concept] = {
            "accuracy": acc,
        ...
        # Keep only items the model got right in baseline
    ...
    # =================================================================
    # BASELINE FILTERING — keep concepts with >= MIN_CORRECT correct items
    # =================================================================
    MIN_CORRECT = 5  # Need at least 5 correct items to measure a deficit
        ...
        for cav_method in CAV_METHODS:
            for layer_label, layers in layer_conditions:
                for alpha in ALPHAS:
                    ...
                    # Evaluate on-target + sampled off-targets
    ...
    output = {
        "metadata": {
            "test": "BEA Naming (Confrontation Naming)",
    ...
    print(f"\n{'='*60}")
    print("BEA NAMING TEST COMPLETE")
    print(f"{'='*60}")
