"""
CCT Test — Camel and Cactus Test (Semantic Association with Ablation)
=====================================================================
Simulates the CCT / Pyramids & Palm Trees neuropsychological test on
...
Input:  Benchmark_construction/output/bea_association_benchmark.json
        cavs/*.pt (pre-trained CAVs)
Output: results/cct_test_results.json

Usage (inside Docker):
    docker run --gpus all -it tesis-gpt2 python -m test.cct_test
...
# Ensure parent directory is in path (needed when running as python -m test.cct_test)
...
BENCHMARK_PATH = os.path.join(
    ...
    "Benchmark_construction", "output", "bea_association_benchmark.json",
)
...
def evaluate_with_hooks(model, dataset, fwd_hooks):
    ...
    if not fwd_hooks:
        return evaluate_bea_association(model, dataset)
...
def main():
    ...
    print("=" * 60)
    print("CCT Test — Semantic Association with Ablation")
    print("=" * 60)
    ...
    # Filter to concepts that exist in the benchmark
    available = [c for c in CONCEPTS if c in items_by_concept]
    missing = [c for c in CONCEPTS if c not in items_by_concept]
    if missing:
        print(f"  WARNING: {len(missing)} concepts not in benchmark: {missing}")
    assert available, "No concepts from cfg.CONCEPTS found in benchmark!"

    for c in available:
        print(f"  {c}: {len(items_by_concept[c])} items")
    ...
    for concept in available:
        acc, per_item = evaluate_bea_association(model, items_by_concept[concept])
        baselines[concept] = {
            "accuracy": acc,
    ...
    output = {
        "metadata": {
            "test": "CCT (Camel and Cactus Test)",
