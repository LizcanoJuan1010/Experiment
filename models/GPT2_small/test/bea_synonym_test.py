"""
BEA Synonym vs Antonym Discrimination Test
============================================
Tests whether GPT-2 small can discriminate synonym pairs from antonym
...
Usage (inside Docker):
    docker run --gpus all -it tesis-gpt2 python -m test.bea_synonym_test

Prerequisites:
    - CAVs extracted at layers [6, 9, 10] for all concepts
...
def wrap_words_as_sentences(pairs):
    ...
    evaluate_r2_ranking expects sentence pairs, not bare words.
    Using a neutral template so the embedding captures word meaning
    in context rather than token-level noise.
    """
    return [
...
def main():
    ...
    print("=" * 60)
    print("BEA Synonym vs Antonym Discrimination Test")
    print("=" * 60)
    ...
    # Include all concepts that have sufficient data (entity + category)
        ...
        for cav_method in CAV_METHODS:
            for layer_label, layers in layer_conditions:
                for alpha in ALPHAS:
                    ...
                    # ON-TARGET: measure ablated concept
                    ...
                    # OFF-TARGET: sample other concepts
