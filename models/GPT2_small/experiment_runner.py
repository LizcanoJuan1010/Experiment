...
def main():
    ...
    results_rows = []
    peritem_rows = []  # Per-item R1 results for rigorous ANOVA
    condition_num = 0
            ...
            for layer in LAYERS:
                ...
                for technique in TECHNIQUES:
                    for alpha in INTENSITIES:
                        ...
                        r1_acc, r2_sim, r1_items, _ = run_condition(
                        ...
                        # Collect per-item R1 results for ANOVA
    ...
    # Save per-item R1 results (1 row per question per condition) for ANOVA
    if peritem_rows:
        df_peritem = pd.DataFrame(peritem_rows)
        peritem_path = os.path.join(RESULTS_DIR, "results_factorial_peritem.csv")
        df_peritem.to_csv(peritem_path, index=False)
        print(f"  Saved: {peritem_path} ({len(df_peritem)} rows)")
